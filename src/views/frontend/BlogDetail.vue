    ...
    <div class="pt-4 pt-lg-8 pb-10 pb-lg-20">
      ...
      <div class="row text-dark gy-8">
        <template v-if="!filterBlogs.length && !filterTags.length">
          <div class="col-lg-8">
            <article>
              ...
              <ul class="list-inline text-gray-dark my-5 d-flex align-items-center flex-wrap">
                ...
                <li class="list-inline-item">
                  <div class="d-flex align-items-center">
                    <span v-for="tag in article.tag" :key="tag" class="d-flex align-items-center me-1"><span class="material-icons fs-6"> local_offer </span>{{ tag }}</span>
                  </div>
                </li>
              </ul>
        ...
        <div class="col-lg-3 offset-lg-1">
          <div class="mb-5 mb-lg-10 position-relative">
            <label for="search" class="form-label mb-2 mb-lg-4"><h5>搜尋</h5></label>
            <input id="search" type="text" class="form-control" v-model="searchInput" placeholder="請輸入關鍵字" aria-label="請輸入關鍵字" @keyup.enter="searchBlogs" />
            <a href="#" @click.prevent="searchBlogs"><span class="material-icons position-absolute text-gray-dark search-icon"> search </span></a>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      searchBlogs() {
        this.filterBlogs = this.articles.filter((item) => item.title.match(this.searchInput.trim()));
        this.searchInput = '';
        this.$route.params.id = ''; // 重要
      },
      searchTag(tag) {
